...
const SignIn1 = ({ onGithubSignIn, onDemoSignIn, loading, demoLoading, authError }: SignInProps) => {
  return (
    <div className="min-h-screen flex flex-col items-center justify-center bg-[#0B0F17] relative overflow-hidden w-full">
      ...
      <div className="relative z-10 w-full max-w-sm rounded-3xl bg-gradient-to-br from-white/[0.07] to-[#0B0F17]/80 backdrop-blur-sm shadow-2xl p-8 flex flex-col items-center border border-white/[0.08]">
        ...
        <button
          ...
          className="w-full flex items-center justify-center gap-2 bg-white/[0.06] hover:bg-white/[0.12] text-gray-300 hover:text-white font-medium px-5 py-3.5 rounded-full border border-white/10 hover:border-white/20 transition-all text-sm disabled:opacity-50 disabled:cursor-not-allowed"
        >
      ...
      <div className="relative z-10 mt-12 flex flex-col items-center text-center">
        <p className="text-gray-400 text-sm mb-3">
          Join <span className="font-medium text-white">500+</span> sales teams already using RepLog AI.
        </p>
        <div className="flex -space-x-2">
          {[
            ...
            <img key={i} src={src} alt="user" className="w-8 h-8 rounded-full border-2 border-[#0B0F17] object-cover" />
          ))}
